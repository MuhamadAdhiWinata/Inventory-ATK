        ...
        <div class="relative z-10 w-full max-w-lg mx-4 bg-card rounded-lg shadow-xl border border-border">
          ...
          <div class="flex items-center justify-between p-6 border-b border-border">
            <div>
              <h2 class="text-lg font-semibold text-card-foreground">
                {{ isEditMode ? 'Edit Transaksi' : 'Tambah Transaksi Masuk' }}
              </h2>
              <p class="text-sm text-muted-foreground mt-0.5">
                {{ isEditMode ? 'Ubah data transaksi' : 'Catat barang masuk ke gudang' }}
              </p>
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1.5">
                <label class="text-sm font-medium text-card-foreground">Gudang</label>
                <select
                  :value="form.gudangId"
