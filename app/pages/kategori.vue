  ...
  <main class="p-4 md:p-6">
    <div class="flex items-center justify-between border-b pb-4 mb-6">
      <div class="flex items-center gap-3">
        <div class="h-10 w-10 rounded-lg bg-violet-100 dark:bg-violet-900/20 flex items-center justify-center">
          <Layers2Icon class="h-5 w-5 text-violet-600" />
        </div>
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Kategori</h1>
          <p class="text-muted-foreground text-sm">Kelola kategori barang.</p>
        </div>
      </div>
    ...
    <div v-else class="border rounded-lg overflow-hidden">
      <table class="w-full text-sm">
        <thead class="border-b bg-muted/50">
          <tr>
            <th class="h-11 px-4 text-left font-medium text-muted-foreground">Nama Kategori</th>
            <th class="h-11 px-4 text-center font-medium text-muted-foreground">Sub Kategori</th>
            <th class="h-11 px-4 text-left font-medium text-muted-foreground">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="kategori in daftarKategori" :key="kategori.id" class="border-b transition-colors hover:bg-muted/50">
            <td class="p-4 font-medium">{{ kategori.name }}</td>
            <td class="p-4 text-center">
              <span class="inline-flex items-center rounded-full bg-muted px-2 py-0.5 text-xs font-medium">
                {{ kategori.jumlahSubKategori }} sub kategori
              </span>
            </td>
            <td class="p-4">
              <div class="flex gap-2">
                <button @click="bukaFormEdit(kategori)" class="h-8 w-8 inline-flex items-center justify-center rounded-md border border-input bg-background hover:bg-accent transition-colors">
                  <PencilIcon class="h-4 w-4" />
                </button>
                <button @click="bukaDialogHapus(kategori)" class="h-8 w-8 inline-flex items-center justify-center rounded-md border border-destructive/20 bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors">
      ...
      <div v-if="daftarKategori.length === 0" class="text-center py-12 text-sm text-muted-foreground">
        Belum ada kategori.
      </div>
    </div>
        ...
        <div v-if="formTerbuka" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative z-10 w-full max-w-md mx-4 bg-card rounded-lg shadow-xl border border-border">
            <div class="flex items-center justify-between p-6 border-b">
              <h2 class="text-lg font-semibold">{{ isEditMode ? 'Edit Kategori' : 'Tambah Kategori' }}</h2>
            ...
            <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
              <div class="space-y-1.5">
                ...
                <input v-model="form.name" type="text" placeholder="Contoh: Alat Tulis"
    ...
    <ConfirmDialog
      ...
      :description="`Hapus kategori '${itemAkanDihapus?.name}'? Semua sub kategori di dalamnya harus dihapus terlebih dahulu.`"
...
import { Layers2Icon, PlusIcon, PencilIcon, Trash2Icon, XIcon } from 'lucide-vue-next'
import ConfirmDialog from '@/components/ConfirmDialog.vue'
import { useToast } from '@/hooks/use-toast'

const { toast } = useToast()

interface Kategori { id: number; name: string; jumlahSubKategori: number }
...
const itemDipilih = ref<Kategori | null>(null)

const dialogHapusTerbuka = ref(false)
const itemAkanDihapus = ref<Kategori | null>(null)
...
const ambilData = async () => {
  ...
  try {
    daftarKategori.value = await $fetch<Kategori[]>('/api/categories')
  } catch {
    pesanError.value = 'Gagal memuat data kategori.'
  } finally {
...
function bukaFormEdit(item: Kategori) {
...
function validate(): boolean {
  Object.keys(errors).forEach(k => delete errors[k])
  if (!form.name.trim()) errors.name = 'Nama kategori wajib diisi'
...
async function handleSubmit() {
  ...
  try {
    if (isEditMode.value && itemDipilih.value) {
      await $fetch(`/api/categories/${itemDipilih.value.id}`, { method: 'PUT', body: form })
      toast({ title: 'Berhasil', description: 'Kategori berhasil diperbarui', variant: 'success' })
    } else {
      await $fetch('/api/categories', { method: 'POST', body: form })
      toast({ title: 'Berhasil', description: 'Kategori berhasil ditambahkan', variant: 'success' })
    }
    formTerbuka.value = false
    ambilData()
  } catch (err: any) {
...
function bukaDialogHapus(item: Kategori) {
...
async function handleHapus() {
  ...
  try {
    await $fetch(`/api/categories/${itemAkanDihapus.value.id}`, { method: 'DELETE' })
