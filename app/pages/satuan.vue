  ...
  <main class="p-4 md:p-6">
    <div class="flex items-center justify-between border-b pb-4 mb-6">
      <div class="flex items-center gap-3">
        <div class="h-10 w-10 rounded-lg bg-teal-100 dark:bg-teal-900/20 flex items-center justify-center">
          <RulerIcon class="h-5 w-5 text-teal-600" />
        </div>
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Satuan</h1>
          <p class="text-muted-foreground text-sm">Kelola satuan barang.</p>
        </div>
      </div>
    ...
    <div v-else class="border rounded-lg overflow-hidden">
      <table class="w-full text-sm">
        <thead class="border-b bg-muted/50">
          <tr>
            <th class="h-11 px-4 text-left font-medium text-muted-foreground">Nama Satuan</th>
            <th class="h-11 px-4 text-center font-medium text-muted-foreground">Digunakan Oleh</th>
            <th class="h-11 px-4 text-left font-medium text-muted-foreground">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in daftarSatuan" :key="item.id" class="border-b transition-colors hover:bg-muted/50">
            ...
            <td class="p-4">
              <div class="flex gap-2">
                ...
                <button @click="bukaDialogHapus(item)" class="h-8 w-8 inline-flex items-center justify-center rounded-md border border-destructive/20 bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors"
                  :disabled="item.jumlahBarang > 0" :title="item.jumlahBarang > 0 ? 'Masih digunakan oleh barang' : ''">
      ...
      <div v-if="daftarSatuan.length === 0" class="text-center py-12 text-sm text-muted-foreground">
        Belum ada satuan.
      </div>
    </div>
        ...
        <div v-if="formTerbuka" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative z-10 w-full max-w-md mx-4 bg-card rounded-lg shadow-xl border border-border">
            <div class="flex items-center justify-between p-6 border-b">
              <h2 class="text-lg font-semibold">{{ isEditMode ? 'Edit Satuan' : 'Tambah Satuan' }}</h2>
            ...
            <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
              <div class="space-y-1.5">
                <label class="text-sm font-medium">Nama Satuan <span class="text-destructive">*</span></label>
                <input v-model="form.name" type="text" placeholder="Contoh: Pcs, Rim, Box"
    ...
    <ConfirmDialog
      ...
      title="Hapus Satuan"
      :description="`Hapus satuan '${itemAkanDihapus?.name}'?`"
...
import { RulerIcon, PlusIcon, PencilIcon, Trash2Icon, XIcon } from 'lucide-vue-next'
import ConfirmDialog from '@/components/ConfirmDialog.vue'
import { useToast } from '@/hooks/use-toast'

const { toast } = useToast()

interface Satuan { id: number; name: string; jumlahBarang: number }

const daftarSatuan = ref<Satuan[]>([])
...
const itemDipilih = ref<Satuan | null>(null)

const dialogHapusTerbuka = ref(false)
const itemAkanDihapus = ref<Satuan | null>(null)
...
const ambilData = async () => {
  ...
  try {
    daftarSatuan.value = await $fetch<Satuan[]>('/api/units')
  } catch {
    pesanError.value = 'Gagal memuat data satuan.'
  } finally {
...
function bukaFormEdit(item: Satuan) {
...
function validate(): boolean {
  Object.keys(errors).forEach(k => delete errors[k])
  if (!form.name.trim()) errors.name = 'Nama satuan wajib diisi'
...
async function handleSubmit() {
  ...
  try {
    if (isEditMode.value && itemDipilih.value) {
      await $fetch(`/api/units/${itemDipilih.value.id}`, { method: 'PUT', body: form })
      toast({ title: 'Berhasil', description: 'Satuan berhasil diperbarui', variant: 'success' })
    } else {
      await $fetch('/api/units', { method: 'POST', body: form })
      toast({ title: 'Berhasil', description: 'Satuan berhasil ditambahkan', variant: 'success' })
    }
    formTerbuka.value = false
    ambilData()
  } catch (err: any) {
...
function bukaDialogHapus(item: Satuan) {
...
async function handleHapus() {
  ...
  try {
    await $fetch(`/api/units/${itemAkanDihapus.value.id}`, { method: 'DELETE' })
    toast({ title: 'Berhasil', description: 'Satuan berhasil dihapus', variant: 'success' })
